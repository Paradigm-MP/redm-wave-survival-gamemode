RobotBoss = class(WaveSurvivalActor)

function RobotBoss:__init()
    self:Initialize()
end

function RobotBoss:Initialize(model)
    self.model = ActorModelEnum.Robot
    self.weapon = EnemyWeapons:GetRandomWeaponForRound(GameManager:GetRoundNumber())

    self:InitializeWaveSurvivalActorFromRobotBoss() -- allow the inherited classes to do initialization
    self:ConfigPed()
...
function RobotBoss:Ready()
    --Chat:Print("Entered RobotBoss:Ready()")
    ...
    if self:LocalPlayerHasControl() then -- this Get call doesnt do ped natives calculations (which is good, it relies on our own data)
        ...
        self:ApplyPedConfig(RobotBossConfig)
...
function RobotBoss:PlayerDowned(args)
...
function RobotBoss:tostring()
    return "RobotBoss (" .. self:GetPedId() .. ")"
end