Wolf = class(WaveSurvivalActor)

function Wolf:__init()
    self:Initialize()
end

function Wolf:Initialize(model)
    self.model = ActorModelEnum.Wolf
    self.weapon = nil
...
function Wolf:Ready()
    ...
    if self:LocalPlayerHasControl() then -- this Get call doesnt do ped natives calculations (which is good, it relies on our own data)
        ...
        self:ApplyPedConfig(WolfConfig)
...
function Wolf:PlayerDowned(args)
...
function Wolf:tostring()
    return "Wolf (" .. self:GetPedId() .. ")"
end