SwampFreak = class(WaveSurvivalActor)

function SwampFreak:__init()
    self:Initialize()
end

function SwampFreak:Initialize(model)
    self.model = ActorModelEnum.SwampFreak
    self.weapon = nil
...
function SwampFreak:Ready()
    --Chat:Print("Entered SwampFreak:Ready()")
    ...
    if self:LocalPlayerHasControl() then -- this Get call doesnt do ped natives calculations (which is good, it relies on our own data)
        ...
        self:ApplyPedConfig(SwampFreakConfig)
...
function SwampFreak:PlayerDowned(args)
...
function SwampFreak:tostring()
    return "SwampFreak (" .. self:GetPedId() .. ")"
end