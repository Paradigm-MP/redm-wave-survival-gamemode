Vampire = class(WaveSurvivalActor)

function Vampire:__init()
    self:Initialize()
end

function Vampire:Initialize(model)
    self.model = ActorModelEnum.Vampire
    self.weapon = WeaponEnum.KnifeVampire
...
function Vampire:Ready()
    --Chat:Print("Entered Vampire:Ready()")
    ...
    if self:LocalPlayerHasControl() then -- this Get call doesnt do ped natives calculations (which is good, it relies on our own data)
        ...
        self:ApplyPedConfig(VampireConfig)
...
function Vampire:PlayerDowned(args)
...
function Vampire:tostring()
    return "Vampire (" .. self:GetPedId() .. ")"
end