RobotWaveDelegator = class(WaveDelegator)
RobotWaveDelegator.survive_until_time = 160
RobotWaveDelegator.survive_until_chance = 34

function RobotWaveDelegator:__init(wave_type_enum, round_data)
...
function RobotWaveDelegator:Configure()
    ...
    elseif self.wave_type_enum == WaveTypeEnum.Boss then
        self.delegation_delay = 18000 / self.number_of_players
        self.spawn_quota = math.ceil(SpawnManager:GenerateRoundEnemyQuota(self.round_number, self.difficulty, self.number_of_players) * .15)
        self.max_alive_at_once = 50
...
function RobotWaveDelegator:Delegate()
...
function RobotWaveDelegator:DelegatePlayer()
    ...
    if least_delegated_to_player then
        ...
        local agent_profile = self:IsBossRound() and AgentProfileEnum.RobotBoss or AgentProfileEnum.BasicRobot
        local spawn_strategy = self:IsBossRound() and SpawnStrategyEnum.RobotBossWave or SpawnStrategyEnum.RobotWave
...
function RobotWaveDelegator:ActorKilled(args)
...
function RobotWaveDelegator:PlayerQuit(args)
...
function RobotWaveDelegator:CheckForEndOfRound()
    if GameManager:GetRoundNumber() == self.round_number then
        if self:IsSurviveUntilRound() and self.round_timer:GetSeconds() > RobotWaveDelegator.survive_until_time then
        ...
        elseif not self:IsSurviveUntilRound() then
            if self.killed >= self.spawn_quota then
                Chat:Broadcast({text = "Reached quota on robot wave"})
                self:SetActive(false)
                GameManager:NewRound()
            end
        end
    else
        Chat:Broadcast({text = "RobotWaveDelegator checking for end of round in wrong round"})
    end
end

function RobotWaveDelegator:GetNumberAlive()
    return self.spawned - self.killed
end

function RobotWaveDelegator:tostring()
    return "BasicRobotWaveDelegator #" .. tostring(self.round_number) .. " (" .. WaveTypeEnum:GetDescription(self:GetWaveTypeEnum()) .. ")"
end