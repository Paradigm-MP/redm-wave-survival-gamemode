...
function UIInstance:Subscribe(event, callback)
    RegisterNUICallback(event, function(args)
        callback(args)
    end)
end
...
function UI:__init()
    ...
    self.ui_ready = false
    RegisterNUICallback("ui/ready", function()
        self.ui_ready = true
    end)
...
function UI:CallEvent(args)
    if args.name and self.uis[args.name] then
        self.uis[args.name]:CallEvent(args.data)
    else
        for name, ui in pairs(self.uis) do
            ui:CallEvent(args.nui_event, args.data)
        end
    end
end
